...
# Use server-side apply to avoid huge last-applied annotations on CRDs
if ! kubectl apply --server-side --force-conflicts -n argocd -f "$ARGOCD_INSTALL_URL"; then
    echo -e "${YELLOW}Server-side apply failed. Retrying after CRD annotation cleanup...${NC}"
    kubectl annotate crd applications.argoproj.io kubectl.kubernetes.io/last-applied-configuration- --overwrite || true
    kubectl annotate crd appprojects.argoproj.io kubectl.kubernetes.io/last-applied-configuration- --overwrite || true
    kubectl annotate crd applicationsets.argoproj.io kubectl.kubernetes.io/last-applied-configuration- --overwrite || true
...
echo "   docker build -f argocd-demo-app/docker/backend.Dockerfile -t argocd-demo-backend:1.0.0 argocd-demo-app"
echo "   docker build -f argocd-demo-app/docker/frontend.Dockerfile -t argocd-demo-frontend:1.0.0 argocd-demo-app"
echo ""
